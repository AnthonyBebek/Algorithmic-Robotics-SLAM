...
class OccupancyGrid:
    ...
    def world_to_grid(self, x: float, y: float) -> Tuple[int, int]:
        ...
        ''' # Find where I am, reletive to origin
            then divide by resolution to find the cell position in the grid
            rather than x and y distances.
            then convert to int to get cell index as you can't have a fraction of a cell.
        '''
    ...
    def update(self, pose: np.ndarray, ranges: np.ndarray,
        ...
        # Compute lidar position in robot frame (apply mounting offset)
        ...
        for i, r in enumerate(ranges):
            ...
            """Beam_angle is the angle of the lidar at that point, r is the distance the lidar is reading
            this is converting the measured point from the lidar to the world frame, so we can figure out
            which cell it corresponds to in the grid"""
