...
def compute_motion_covariance(relative_pose: np.ndarray,
    ...
    delta_trans = np.sqrt(relative_pose[0]**2 + relative_pose[1]**2) # Pythagorean theorem to compute translation distance
    delta_rot = np.abs(relative_pose[2]) # Absolute value of rotation angle to compute how much we turned
    var_x = alpha1 * delta_trans**2 + alpha2 * delta_rot**2 # Translational noise from translation + rotation
    var_y = alpha1 * delta_trans**2 + alpha2 * delta_rot**2 # Same as var_x since we assume isotropic noise in x and y directions
    var_theta = alpha3 * delta_trans**2 + alpha4 * delta_rot**2 # Rotational noise from translation + rotation
    cov = np.diag([var_x, var_y, var_theta]) # Make the diagonal covariance matrix
